...
class TwitterV2Impl(private val twitter: Twitter) : TwitterV2 {
    ...
    override fun uploadMediaChunkedAppend(mediaId: Long, segmentIndex: Long, fileName: String, media: InputStream) {
        val params = arrayListOf(
            ...
            // 旧: HttpParameter("media", fileName, media)（InputStream コンストラクタは JVM 依存）
            V2MediaUploadSupport.mediaHttpParameter("media", fileName, media),
        )
    ...
    override fun uploadMedia(mediaType: String, fileName: String, media: InputStream): LongResponse {
        // 旧: media.readBytes()（IOException→TwitterException 変換込み）を V2MediaUploadSupport へ移設
        val dataBytes = V2MediaUploadSupport.readBytes(media).also {
        ...
        val dataInputStream = V2MediaUploadSupport.byteArrayInputStream(dataBytes)
        var segmentIndex = 0L

        while (true) {
            val segmentData = V2MediaUploadSupport.readNBytes(dataInputStream, CHUNK_SIZE)
            if (segmentData.isEmpty()) break
            uploadMediaChunkedAppend(response.mediaId, segmentIndex++, fileName, V2MediaUploadSupport.byteArrayInputStream(segmentData))
        }
